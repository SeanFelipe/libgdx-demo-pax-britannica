...
class Ship (iid: Int, pposition: Vector2, ffacing: Vector2, hp: Int = 0) extends Sprite() {
    ...
    def goTowardsOrAway(targetPos: Vector2, forceThrust: Boolean, isAway: Boolean) {
        ...
        if (isAway) { target_direction.scl(-1) }

        if (facing.crs(target_direction) > 0) { turn(1) } 
        else { turn(-1) }

        if (forceThrust || facing.dot(target_direction) > 0) { thrust() }
    }
    ...
    def factoryDestruct() {
        ...
        if (deathCounter > 0) {
            ...
            for (i <- 1 to 10)  { GameInstance.explode(this, randomPointOnShip) }
            alive = false
        }
    }
